...
func calculateAsleepyness(records []rec) {
	...
	// 04.02
	// for all guards, which is asleep most on the same minute?
	// Output (GuardID * Minute)
	guardminutes := make(map[int][]int)
	for _, r := range records {
		if strings.Contains(r.Entry, "begins shift") { // only sleeps and wakes needed
			guard = r.GuardID
			if guardminutes[guard] == nil {
				guardminutes[guard] = make([]int, 60) // prepopulate 60
			}
			if guardminutes[sleepiestGuard] == nil {
				guardminutes[sleepiestGuard] = make([]int, 60)
			}
		}
		if strings.Contains(r.Entry, "falls asleep") {
			sleepFrom = r.Time.Minute()
			...
			sleepTo = r.Time.Minute()
			for i := sleepFrom; i < sleepTo; i++ {
				guardminutes[guard][i]++
				if guardminutes[guard][i] > guardminutes[sleepiestGuard][mostSleepyMinute] {
					mostSleepyMinute = i
					sleepiestGuard = guard
				}
			}
	...
	fmt.Printf("Sleepiest Guard is #%v, and they slept %vm on minute %v\n",
		sleepiestGuard, guardminutes[sleepiestGuard][mostSleepyMinute], mostSleepyMinute,
	)
	fmt.Printf("Answer 04.02 (%v x %v): %v\n", sleepiestGuard, mostSleepyMinute, sleepiestGuard*mostSleepyMinute)
